...
import com.example.addon.Utils.AutomationUtils;
import com.example.addon.Utils.BlockHelper;
import com.example.addon.Utils.AutomationUtils;
import com.example.addon.Utils.BedUtils;
import com.example.addon.Utils.ItemHelper;
...
public class BedAuraBEBRA extends Module {
    ...
    public BedAuraBEBRA() {
        super(Addon.combat, "bed-aura-plus", "The famous troll module.");
    }

    @Override
    public void onActivate() {
        target = null;
        if (autoMoveSlot.get() - 1 == -1) ogItem = null;
        else if (autoMoveSlot.get() - 1 == 45) ogItem = mc.player.getOffHandStack().getItem();

        else ogItem = mc.player.getInventory().getStack(autoMoveSlot.get() - 1).getItem();
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (preventEscape.get() && BlockHelper.getBlock(target.getBlockPos().up(2)) != Blocks.OBSIDIAN && AutomationUtils.isInHole(target)) {
        ...
        if (breakSelfTrap.get() && shouldTrapMine()) {
            ...
            if (pick.found()) {
                mc.player.getInventory().selectedSlot = pick.slot();
